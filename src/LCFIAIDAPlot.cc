...
//authors: Mark Grimes (Mark.Grimes@bristol.ac.uk), Victoria Martin (victoria.martin@ed.ac.uk)
//some documentation: http://www.ph.ed.ac.uk/~vjm/ILC/LCFI/LCFIAIDAPlot/Plots.html

#ifndef MARLIN_USE_AIDA

#warning "-----------------------------------------------------------------------"
#warning "- LCFIAIDAPlot requires MARLIN_USE_AIDA to be defined. Did you enable -"
#warning "- AIDA when compiling Marlin? LCFIAIDAPlot will not be compiled.      -"
#warning "-----------------------------------------------------------------------"
...
{ 
  ...
  registerOptionalParameter("PTJetMax",
			    "Cut determining the maximum PT of the jet.  Default: 10000 GeV/c"  ,
			    _PTJetMax,
			    double(10000.)) ;
   
  registerOptionalParameter( "PTJetMin",
...
LCFIAIDAPlot::~LCFIAIDAPlot() 
{ 
} 

void LCFIAIDAPlot::init()
{
  ...
  _VertexCatNames[0]="AnyNumberOfVerticies";
  _VertexCatNames[1]="OneVertex";
  _VertexCatNames[2]="TwoVerticies";
  _VertexCatNames[3]="ThreeOrMoreVerticies";
...
void LCFIAIDAPlot::processRunHeader( LCRunHeader* pRun ) 
{
...
void LCFIAIDAPlot::processEvent( LCEvent* pEvent ) 
{ 
...
void LCFIAIDAPlot::check( LCEvent* pEvent ) 
{
}

void LCFIAIDAPlot::end() 
{
  ...
  _pBJetBTagIntegral.resize( _FlavourTagInputsCollectionNames.size() );     
  _pCJetBTagIntegral.resize( _FlavourTagInputsCollectionNames.size() );	  
  _pLightJetBTagIntegral.resize( _FlavourTagInputsCollectionNames.size() ); 
  _pBJetCTagIntegral.resize( _FlavourTagInputsCollectionNames.size() );  	  
  _pCJetCTagIntegral.resize( _FlavourTagInputsCollectionNames.size() );	  
  _pLightJetCTagIntegral.resize( _FlavourTagInputsCollectionNames.size() ); 
  _pBJetBCTagIntegral.resize( _FlavourTagInputsCollectionNames.size() ); 	  
  _pCJetBCTagIntegral.resize( _FlavourTagInputsCollectionNames.size() );	  
  _pLightJetBCTagIntegral.resize( _FlavourTagInputsCollectionNames.size() );
  ...
  for (unsigned int iTagCollection=0; iTagCollection < _FlavourTagCollectionNames.size(); ++iTagCollection )
	...
	//sum over the different vertex catagories, this information goes into the "AnyNumberOfVerticies" directory
...
bool LCFIAIDAPlot::PassesJetCuts( ReconstructedParticle* pJet )
{
  ...
  if (momentumMagnitude > _PTJetMax ||  momentumMagnitude < _PTJetMin) return false;
...
int LCFIAIDAPlot::FindJetPDGCode( LCEvent* pEvent, unsigned int jetNumber )
{
...
float LCFIAIDAPlot::FindJetPartonCharge( LCEvent* pEvent, unsigned int jetNumber )
{
...
int LCFIAIDAPlot::FindJetType( LCEvent* pEvent, unsigned int jetNumber )
{
...
int LCFIAIDAPlot::FindNumVertex( LCEvent* pEvent, unsigned int jetNumber, unsigned int iInputsCollection)
{
...
AIDA::IHistogram1D* LCFIAIDAPlot::CreateIntegralHistogram(const AIDA::IHistogram1D* pNN, AIDA::IHistogram1D* pIntegral)
{
...
AIDA::IDataPointSet* LCFIAIDAPlot::CreateIntegralPlot(const AIDA::IHistogram1D* pNN, AIDA::IDataPointSet* pDataPointSet )
{
...
AIDA::IDataPointSet* LCFIAIDAPlot::CreatePurityPlot(const AIDA::IHistogram1D* pSignal, const AIDA::IHistogram1D* pBackground, AIDA::IDataPointSet* pDataPointSet)  
{
  const int numberOfBins=pSignal->axis().bins();
  int iPoint=0;
...
AIDA::IDataPointSet* LCFIAIDAPlot::CreateLeakageRatePlot(const AIDA::IHistogram1D* pBackground, AIDA::IDataPointSet* pDataPointSet)
{
...
AIDA::IDataPointSet* LCFIAIDAPlot::CreateXYPlot(const AIDA::IDataPointSet* pDataPointSet0, const AIDA::IDataPointSet* pDataPointSet1, AIDA::IDataPointSet* xyPointSet, const int dim0, const int dim1 )
{
...
void LCFIAIDAPlot::PrintNNOutput(){
...
void LCFIAIDAPlot::InternalVectorInitialisation()
{
