...
The method used to create a new network varies slightly depending on the algorithm used to train it.  Sections \ref BuildingTheNeuronLayers and \ref CreatingTheNetwork describe how to setup the network ready for training, which is common to all training algorithms.  The <tt>BatchBackPropagationAlgorithm</tt>, <tt>BackPropagationCGAlgorithm</tt> and <tt>GeneticAlgorithm</tt> algorithms require the training data to be pre-stored in a <tt>nnet::NeuralNetDataSet</tt> class (section \ref BuildingTheTrainingSample), and will train themselves over the whole data set.  <tt>BatchBackPropagationAlgorithm</tt> on the other hand performs one training step at a time to provide more control over each training step.<BR>
...
neuronsInLayer.push\_back( 6 );
neuronsInLayer.push\_back( 4 );
neuronsInLayer.push\_back( 2 );
...
layer1.push\_back( "LinearNeuron" );
layer1.push\_back( "SigmoidNeuron" );
// ...and so on until there are six names in the vector -> six neurons in the layer

std::vector<std::string> layer2;
layer2.push\_back( "TanSigmoidNeuron" );
// ... and so on another three times

std::vector<std::string> outputlayer;
outputlayer.push\_back( "LinearNeuron" );
outputlayer.push\_back( "LinearNeuron" );

std::vector< std::vector<std::string> > neuronNames;
neuronNames.push\_back( layer1 );
neuronNames.push\_back( layer2 );
neuronNames.push\_back( outputlayer );
// The number of inputs is set later.
\endcode
...
// for donkey 1
output.push\_back( 1 ); // I know for certain this animal is a donkey
inputs.push\_back( 4 ); // it has four legs
inputs.push\_back( 1.45 ); // it is 1.45m tall
inputs.push\_back( 0.32 ); // it has a tail 32cm long
...
//for Geoff
output.push\_back( 0 ); // I know for certain Geoff isn't a donkey
inputs.push\_back( 2 ); // he has 2 legs
inputs.push\_back( 1.82 ); // he is 1.82m tall
...
inputs.push\_back( 0 ); // Geoff's tail is 0cm long
...
std::vector<double> inputs;
inputs.push\_back( 4 ); // Fido has four legs
inputs.push\_back( 0.54 ); // he is 54cm tall
inputs.push\_back( 0.25 ); // his tail is 25cm long
